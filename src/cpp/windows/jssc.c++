...
#include <jni.h>
#include <stdlib.h>
#include <windows.h>/* jSSC (Java Simple Serial Connector) - serial port communication library.
* © Alexey Sokolov (scream3r), 2010-2014.
*
* This file is part of jSSC.
*
...
JNIEXPORT jlong JNICALL Java_jssc_SerialNativeInterface_openPort(JNIEnv *env, jobject object, jstring portName, jboolean useTIOCEXCL) {
	...
	// rewritten for 2.8.1 ->
	jint len = (int)(sizeof(prefix) + strlen(port));
	char * portFullName = (char *) malloc(len + 1);
	strcpy_s(portFullName, len, prefix);
	strcat_s(portFullName, len, port);
	//<-rewritten for 2.8.1
...
JNIEXPORT jbyteArray JNICALL Java_jssc_SerialNativeInterface_readBytes
  (JNIEnv *env, jobject object, jlong portHandle, jint byteCount){
	...
	OVERLAPPED *overlapped = new OVERLAPPED();
	// changed for 2.8.1 ->
	jbyteArray temp = env->NewByteArray(byteCount);
	jbyte * lpBuffer = env->GetByteArrayElements(temp, JNI_FALSE);
	// <- changed for 2.8.1
